...
class AdvancedEnsembleExoplanetClassifier:
    ...
    def plot_comprehensive_comparison(self, save_path='comprehensive_comparison.png'):
        ...
        # Prepare data - sort by F1 score for consistency
        model_names = []
        accuracies = []
        f1_scores = []
        auc_scores = []
        ...
        # Calculate dynamic x-axis limits based on data
        min_acc = min(accuracies)
        max_acc = max(accuracies)
        min_f1 = min(f1_scores)
        max_f1 = max(f1_scores)
        min_auc = min([s for s in auc_scores if s > 0]) if any(s > 0 for s in auc_scores) else 0
        max_auc = max(auc_scores)
        
        # Set limits with some padding
        acc_range = max_acc - min_acc
        f1_range = max_f1 - min_f1
        auc_range = max_auc - min_auc if min_auc > 0 else max_auc
        ...
        axes[0].set_xlim(max(0, min_acc - acc_range * 0.1), min(100, max_acc + acc_range * 0.05))
        axes[0].grid(axis='x', alpha=0.3, linestyle='--')
        
        # Add value labels for accuracy
        for i, (bar, val) in enumerate(zip(bars1, accuracies)):
            axes[0].text(val + acc_range * 0.01, i, f'{val:.2f}%', 
        ...
        axes[1].set_xlim(max(0, min_f1 - f1_range * 0.1), min(100, max_f1 + f1_range * 0.05))
        axes[1].grid(axis='x', alpha=0.3, linestyle='--')
        
        # Add value labels for F1
        for i, (bar, val) in enumerate(zip(bars2, f1_scores)):
            axes[1].text(val + f1_range * 0.01, i, f'{val:.2f}%', 
                        va='center', fontsize=8)
        ...
        if min_auc > 0:
            axes[2].set_xlim(max(0, min_auc - auc_range * 0.1), min(100, max_auc + auc_range * 0.05))
        else:
            axes[2].set_xlim(0, min(100, max_auc + auc_range * 0.05))
        axes[2].grid(axis='x', alpha=0.3, linestyle='--')
        
        # Add value labels for AUC
        for i, (bar, val) in enumerate(zip(bars3, auc_scores)):
            if val > 0:
                axes[2].text(val + max(auc_range * 0.01, 0.5), i, f'{val:.2f}%', 
                            va='center', fontsize=8)
            else:
                axes[2].text(1, i, 'N/A', va='center', fontsize=8)
    ...
    def plot_ensemble_comparison(self, save_path='ensemble_comparison.png'):
        ...
        for i, (metric_name, values) in enumerate(metrics.items()):
            ...
            for bar in bars:
                ...
                if height > 0:  # Only show label if value is meaningful
        ...
        all_values = [v for values in metrics.values() for v in values if v > 0]
        if all_values:
            min_val = min(all_values)
            max_val = max(all_values)
            y_range = max_val - min_val
            ax.set_ylim(max(0, min_val - y_range * 0.1), min(100, max_val + y_range * 0.15))
        else:
            ax.set_ylim(90, 100)
