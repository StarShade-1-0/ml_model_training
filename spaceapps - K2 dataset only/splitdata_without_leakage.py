...
def split_data_with_overfitting_prevention(
    ...
    feature_cols = [col for col in df_filtered.columns 
                   if col not in ['pl_name', 'disposition']]
    
    X = df_filtered[feature_cols].copy()
    y = df_filtered['disposition'].copy()
    pl_name = df_filtered['pl_name'].copy()
    ...
    X_train_val, X_test, y_train_val, y_test, pl_name_train_val, pl_name_test = train_test_split(
        X, y, pl_name,
        test_size=test_size,
    ...
    X_train, X_val, y_train, y_val, pl_name_train, pl_name_val = train_test_split(
        X_train_val, y_train_val, pl_name_train_val,
    ...
    if balance_method:
        ...
        y_train_balanced = pd.Series(y_train_balanced, name='disposition')
        
        # Note: We don't have pl_name for synthetic samples, so we'll create dummy IDs
        pl_name_train_balanced = pd.Series(range(len(y_train_balanced)), name='pl_name')
        
        # Use balanced data
        X_train = X_train_balanced
        y_train = y_train_balanced
        pl_name_train = pl_name_train_balanced
    ...
    with open(summary_path, 'w') as f:
        ...
        for split_name, split_df in [('Train', train_df), ('Validation', val_df), ('Test', test_df)]:
            f.write(f"\n{split_name}:\n")
            for label, count in split_df['disposition'].value_counts().items():
...
def verify_split(train_df, val_df, test_df):
    ...
    # Check for overlaps using pl_name
    train_ids = set(train_df['pl_name'])
    val_ids = set(val_df['pl_name'])
    test_ids = set(test_df['pl_name'])
    
    train_val_overlap = train_ids.intersection(val_ids)
    train_test_overlap = train_ids.intersection(test_ids)
    val_test_overlap = val_ids.intersection(test_ids)
    
    print("\nChecking for data leakage (overlapping pl_name):")
